...
// Hoyo
// new pointer fields for v19 saved games
// kept separate for backward compatibility with older savegames
static saveField_t gentityFields_19[] = {
	{FOFS(targetShaderName),                    F_STRING},
	{FOFS(targetShaderNewName),                 F_STRING},
	{FOFS(scriptStatusBackup.animatingParams),  F_STRING},
	{FOFS(scriptStatusCurrent.animatingParams), F_STRING},
...
// new pointer fields for v19 saved games
// kept separate for backward compatibility with older savegames
static saveField_t gclientFields_19[] = {
...
void WriteClient( fileHandle_t f, gclient_t *cl ) {
	...
	// Hoyo
	if (ver >= 19) {
		for (field = gclientFields_19; field->type; field++) {
			WriteField1(field, (byte*)&temp);
		}
	}
...
void ReadClient( fileHandle_t f, gclient_t *client, int size ) {
	...
	if (ver >= 19) {
		...
		// Older savegames stored these as raw process pointers.
		// They cannot safely be restored.
		temp.cameraPortal = NULL;
		temp.persistantPowerup = NULL;
	}
...
void WriteEntity( fileHandle_t f, gentity_t *ent ) {
	...
	// Hoyo. Added new missing fields in v19
	if (ver >= 19) {
		for (field = gentityFields_19; field->type; field++) {
			WriteField1(field, (byte*)&temp);
		}
	}
...
void ReadEntity( fileHandle_t f, gentity_t *ent, int size ) {
	...
	if (ver >= 19) {
		...
		// Do not trust raw pointer values stored by old save versions.
		temp.targetShaderName = NULL;
		temp.targetShaderNewName = NULL;
		temp.scriptStatusBackup.animatingParams = NULL;
		temp.scriptStatusCurrent.animatingParams = NULL;
	}
...
void G_LoadGame( char *filename ) {
	...
	if (i != SAVE_VERSION &&
		...
		i != 13 &&
		i != 14 &&
		i != 15) {    // 13 is beta7, 14 is pre "SA_MOVEDSTUFF". Hoyo: Added v19
		trap_FS_FCloseFile( f );
