...
==================
Added by Hoyo
Save/Lod pos commands
==================
*/
#define LOADPOS_COOLDOWN 100

static int loadPosTime = -LOADPOS_COOLDOWN;
static vec3_t g_savedSpeedrunOrigin;
static vec3_t g_savedSpeedrunAngles;
static vec3_t g_savedSpeedrunVelocity;
static qboolean g_savedSpeedrunPositionValid = qfalse;
static int g_savedSpeedrunHealth;
static int g_savedSpeedrunSprintTime;
static int g_savedSpeedrunSprintExertTime;
...
void Cmd_SavePos_f(gentity_t* ent) {
	...
	VectorCopy(ent->client->ps.origin, g_savedSpeedrunOrigin);
	VectorCopy(ent->client->ps.viewangles, g_savedSpeedrunAngles);
	VectorCopy(ent->client->ps.velocity, g_savedSpeedrunVelocity);

	g_savedSpeedrunHealth = ent->health;
	g_savedSpeedrunSprintTime = ent->client->ps.sprintTime;
	g_savedSpeedrunSprintExertTime = ent->client->ps.sprintExertTime;

	g_savedSpeedrunPositionValid = qtrue;
	loadPosTime = level.time;
...
void Cmd_LoadPos_f(gentity_t* ent) {
	...
	TeleportPlayer(
		ent,
		g_savedSpeedrunOrigin,
		g_savedSpeedrunAngles
	);

	VectorCopy(g_savedSpeedrunVelocity, ent->client->ps.velocity);

	ent->health = g_savedSpeedrunHealth;
	ent->client->ps.stats[STAT_HEALTH] = g_savedSpeedrunHealth;
