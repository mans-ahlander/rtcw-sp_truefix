...
rescan:
	...
	// Hoyo - a cinematic command generated by the previous map must not be replayed by cgame after a full level transition.
	if (clc.gamestateMapChange && serverCommandNumber <= clc.gamestateServerCommandSequence && !strcmp(cmd, "startCam")) {
		//Com_Printf( "Ignoring stale pre-transition camera command %i: %s\n", serverCommandNumber, s );
		return qfalse;
	}
